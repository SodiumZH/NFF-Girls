...
public class BefriendedMeleeAttackGoal extends BefriendedGoal
{
	...
	public BefriendedMeleeAttackGoal(IBefriendedMob pMob, double pSpeedModifier, boolean pFollowingTargetEvenIfNotSeen)
	{
		...
		if (!(pMob instanceof PathfinderMob))
			throw new UnsupportedOperationException("BefriendedMeleeAttackGoal supports only PathfinderMob.");
		...
		} else
		{
			...
			} else
			{
				...
				else
				{
					...
					if (GeometryUtil.getBoxSurfaceDistSqr(this.mob.asMob().getBoundingBox(), livingentity.getBoundingBox()) <= this.getAttackMaxSurfaceDistSqr(livingentity))
						return true;
					else
						return false;
				}
	...
	protected void checkAndPerformAttack(LivingEntity pEnemy) {
		...
		double distSqr = GeometryUtil.getBoxSurfaceDistSqr(mob.asMob().getBoundingBox(), pEnemy.getBoundingBox());
