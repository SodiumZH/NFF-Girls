 ...
 */
public class InventoryTagWithEquipment extends InventoryTag
{
	public InventoryTagWithEquipment(int size)
	{
		super(size);
		if (size < 6)
			throw new IllegalArgumentException("InventoryTagWithEquipment must have at least 6 slots for a mob's equipment slots.");
	}
	
	public void getFromMob(Mob mob)
	{
		this.put(mob.getItemBySlot(EquipmentSlot.HEAD), 0);
		this.put(mob.getItemBySlot(EquipmentSlot.CHEST), 1);
		this.put(mob.getItemBySlot(EquipmentSlot.LEGS), 2);
		this.put(mob.getItemBySlot(EquipmentSlot.FEET), 3);
		this.put(mob.getItemBySlot(EquipmentSlot.MAINHAND), 4);
		this.put(mob.getItemBySlot(EquipmentSlot.OFFHAND), 5);
	}
